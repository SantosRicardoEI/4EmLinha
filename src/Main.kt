...
fun processaColuna(numColunas:Int, coluna:String): Boolean {
    if (numColunas in (1..26) && coluna.length == 1 && coluna[0] in Char(65)..Char(90)) {
        return coluna[0].code - 64 <= numColunas
    }
    return false
}
...
fun criaLinhasTabuleiro(numLinhas: Int,numColunas: Int): String {
    var linhaTabuleiro = "\u2551"
    var count = 0
    while (linhaTabuleiro.length < criaTopoTabuleiro(numColunas-1).length) {
        linhaTabuleiro += "   |"
        count++
    }
    linhaTabuleiro += "   \u2551"
...
fun main() {
    val mostrarLegenda = true                                         //Alterar para mostrar ou não a legenda do tabuleiro
    imprimeMenu()                                                     //Mostra o Menu (Opcoes)
    do {
        ...
        if (opcao == 1) {
            ...
            mostraPorImplementar()                                    //Mostra "POR IMPLEMENTAR"
            opcao = null                                              //e termina o programa (opcao == null termina o ciclo)
        }
        if (opcao == 2) {
